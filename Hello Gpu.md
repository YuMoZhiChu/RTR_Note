...
TES跟顶点着色器十分相似，它只有一个输入（PG产生的质心坐标），以及一个输出（顶点坐标）

它的作用就是，用PG中得到的坐标，再结合patch中的信息（带入曲面多项式中）得到顶点信息。

#### 作用

ts 可以配合一张高度贴图，在 tes 中计算时，做一些细小的采样位移，这样就可以描述更精确的顶点内容了。

![3.11](pic/3/3.11.png)

在巫师3中，ts流程被用作绘制头发。

![W3 Iso Hair](pic/hello_gpu/w3_iso_hair.png)

## 几何着色器

### API

几何着色器在 DX10，OpenGL 3.2，ES 3.2 以上被支持。
...
```
a single object and its associated vertices.
```

怎么理解呢，其实他这里的object，指的就是由基元（primitive）构成的内容。
...
gs还支持一种带 strip 的输入，分别是 line_strip， tri_strip

带了strip，就会自动拓展，就像右边2个一样。

而gs的输出类型，可以是以上输入类型中的任意一种，再加上，输出0。

所以通过gs，很容易做到对一个网格，选择性的修改，增删顶点。

![Gs Norline Learnopengl](pic/hello_pipeline/gs_norline_learnopengl.png)

### 特点

gs 可以做到对输入数据进行修改，或者做一定数量的拷贝，因此可以实现一些渲染上的功能。

![3.13](pic/3/3.13.png)

gs 能够保证输出的基元结果跟他们输入的顺序一样，这是优点，也是缺点。

缺点在于，对于GPU的并行，结果需要保存并记录顺序。

## 流输出

流输出（stream output) 在 GL中被称为 transform feedback，以为这可以将处理好的顶点再次送回管线进行处理

![Ts Gl](pic/hello_gpu/ts_gl.png)

甚至，我们可以将光栅化的阶段关闭，将这个循环结构用作其他的数据处理，而不仅仅是渲染图片。

## 像素着色器

![3.2](pic/3/3.2.png)

在像素着色器的前一步，是光栅化。这一步是属于 configurable。

>* 光栅化生成了片元，片元是基元部分或者全部覆盖一个像素的内容。
>* 片元的属性，在光栅化的过程中，由三角形（或者线段）插值而来。
>* 这些片元，就是ps的输入。

ps的作用就是通过输入，计算并输出一个片元的颜色值。

ps可以修改光栅化阶段的深度值，DX11.3中，可以修改模板缓冲值。

在SM4.0 之后，雾计算和alpha测试也从 merge阶段转移到了 像素着色器阶段。

### 丢弃

ps 可以直接选择 discard，这样会将这个片元直接丢弃。达到一些截取效果。
...
最初，ps只能输出到 merge 阶段，但随着 ps 指令越来越多，人们不满足于只渲染到一个目标上。

于是推出了 MRT，所谓RT，其实就是一张缓存（一般来说，这张缓存是和原来的内容同样x，y大小的）

缓存的位深度可以自己甚至，可以是8位，24位等，依照需求来确定。

根据GPU的不同，RT通常来说是4个或者8个。

MRT 的一个最经典的例子就是 G-Buffer + 后渲染。

![After Renfer G Buffer](pic/hello_gpu/after_renfer_G_buffer.png)

### 限制与特性

ps的限制就是，只能处理当前位置的片元，但不能读取到旁边像素值的信息。

不过可以用多个pass来绕过这个限制，即一个pass先输出中间图片，作为下一个pass的输入数据。

**例外**

在计算梯度和导数信息时，ps 能**间接**获得邻近片元的信息。

ps 提供了沿着屏幕x轴和y轴的插值变化信息（类似 ddx，ddy这种函数）。（用处之后再说）

![3.15](pic/3/3.15.png)

所有的现代GPU，都是以 2*2 一组，称之为 Quad，来处理片元。当 PS 请求一个梯度值时，就会返回对应的值。

比如图中的三角形，光栅化后虽然只剩一个片元，但仍然需要一个 Quad 来获得梯度信息。

*个人理解，一个 Quad 中的梯度信息，都是一样的。*

一个标准核心能过获取同一组内相邻线程的数据，这样保证了梯度的计算。

不过这就需要，所有的线程在获取梯度期间，执行的是相同的指令（没有if-loop），这样计算的梯度才有意义。

**UAV**

DX11 推出了 UAV （unordered access view），在 OpenGl 中被称为 shader storage buffer object (SSBO)

这是一个能在任意位置进行写访问的 buffer，非常强大（在DX11，只有ps和cs可以）

当然，这里会有可能的读写冲突，所以GPU用专门的原子操作访问（同时导致了可能发生的等待）

**ROV**

Rasterizer order views (ROVs)，可以通过这个来确定ps的执行顺序，在ps中实现颜色的混合，而不需要merge阶段。

**TODO：具体的使用方法目前未知**

## 合并阶段

合并阶段是将单个片元的颜色和深度，进行合并，并输出。

其实对于不透明的表面，只是替换，合并只在透明的表面上出现。

### early-z

early-z 是一个提前做深度测试的技术，具体细节这里不了。

主要就是，提前丢弃一些片元，不必要在做这些片元的大量计算，来提升效率（其实G-Buffer也算是一种 early-z)

### 渲染合并

一般来说，合并阶段就是2个颜色的合并操作，一般来说，有以下操作。

![Dx11 Longshu Blend](pic/hello_gpu/dx11_longshu_blend.png)

*取自DX11龙书*

在DX 10.1之后，推出了 MRT + blend 的功能。

## 计算着色器

cs 是一种特殊的着色器，它并仅仅用于传统的图形管线，还可以应用在非图形的领域。

即，GPU计算，有许多相关的库，CUDA，OpenCL等。

### 如何工作

**调用**

cs 直接由API调用，它没有确定在管线的哪个位置，可以和vs，ps一起使用。

![Witch3 Cs Call](pic/hello_gpu/witch3_cs_call.png)

*最后一个白块，就是cs*

**工作**

它和其他着色器一样，使用相同的 标准着色器处理器 池。

输入是一组数据（往往是一张图片），可以访问缓冲区（纹理等），一般来说会等价输出一张图片。

![3.15.1](pic/3/3.15.1.png)

它的分组分为 3个维度组成的线程组

每个线程组也有3个维度，单个(x,y,z)就是一个线程（高强度的高并发结构）

在巫师3中，输入线程组 * 每组线程数，就是输出图片的大小，一般来说，也是一个线程处理一个像素。

![Witch3 Cs Detal](pic/hello_gpu/witch3_cs_detal.png)

而输出大小：

![Witch3 Cs Detal2](pic/hello_gpu/witch3_cs_detal2.png)
